...
export const StartDecision = () => {
    ...
    const handleStart = async (e: React.FormEvent) => {
        ...
        setError(null);
        try {
            const response = await api.post('/decisions', { title, status: 'DRAFT', criteria: [], options: [] });
            setDecision(response.data);
            navigate('/define-criteria');
        } catch (err) {
            console.error('Error al crear decisión:', err);
            setError('Hubo un problema al crear la decisión. Inténtalo de nuevo.');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="max-w-2xl mx-auto mt-10">
            <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 transition-colors duration-300">
                ...
                {error && (
                    <div className="mb-4 p-4 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 rounded-lg text-sm border border-red-100 dark:border-red-800">
                        {error}
                    </div>
                )}
                ...
                <form onSubmit={handleStart} className="space-y-6">
                    <div>
                        ...
                        <input
                            ...
                            disabled={isLoading}
                            className="w-full px-4 py-3 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all disabled:opacity-50"
                            autoFocus
                        />
                    </div>
                    <button
                    ...
                    >
                        {isLoading ? <><Loader2 className="w-5 h-5 animate-spin" /> Creando...</> : 'Comenzar Análisis Estructurado'}
                    </button>
                </form>
            </div>
        </div>
