...
export const PendingFeedbackBanner = () => {
        ...
        const fetchPending = async () => {
            try {
                ...
                if (res.data.length > 0) {
                    ...
                    setActiveDecision(res.data[0]); // Mostramos la más antigua primero
                }
            } catch (error) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            toast.success("¡Gracias por tu feedback! El modelo ahora es más inteligente.");

            // Removemos la decisión actual de la lista
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center gap-8">
                <div className="flex-1 text-white">
                    <span className="bg-indigo-500/30 border border-indigo-400/30 text-indigo-200 text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full mb-3 inline-block">
                        Retrospectiva Requerida
                    </span>
                    <h3 className="text-2xl font-black mb-2">¿Qué tal resultó esta decisión?</h3>
                    <p className="text-indigo-200 text-sm mb-4">
                        Hace un tiempo decidiste <span className="text-white font-bold underline decoration-indigo-500">"{activeDecision.recommendedOption?.name}"</span> para resolver el dilema "{activeDecision.title}". Ayuda a calibrar nuestro ranking marcando tu nivel de satisfacción.
                    </p>
                </div>
