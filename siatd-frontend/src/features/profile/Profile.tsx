...
export const Profile = () => {
    ...
    const formatDate = (dateStr: string) => {
        if (!dateStr) return t('profile.notRegistered');
    ...
    if (isFetching) return (
        <div className="flex flex-col items-center justify-center h-64">
            ...
            <p className="mt-4 text-slate-500 font-medium">{t('profile.loading')}</p>
        </div>
    );
    ...
    return (
        <div className="max-w-4xl mx-auto p-6 animate-in fade-in duration-500">
            <div className="flex justify-between items-center mb-8">
                <h1 className="text-3xl font-bold text-slate-800 dark:text-white">{t('profile.title')}</h1>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden transition-all duration-300">
                ...
                <div className="pt-20 p-8 space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-2 md:col-span-2">
                            <div className="flex justify-between items-center ml-1">
                                ...
                                {isEditing && <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded-full flex items-center gap-1"><Lock className="w-3 h-3" /> {t('profile.notEditable')}</span>}
                            </div>
                    ...
                    <div className="p-5 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-500/10 dark:to-teal-500/10 border border-emerald-100 dark:border-emerald-500/20 rounded-2xl flex items-start sm:items-center gap-4 transition-all">
                        ...
                        <div>
                            <p className="text-sm font-bold text-emerald-900 dark:text-emerald-400 mb-0.5">{t('profile.verified')}</p>
                            <p className="text-xs text-emerald-700 dark:text-emerald-500 font-medium leading-relaxed">{t('profile.verifiedDesc')}</p>
                        </div>
                    </div>
                </div>
            </div>
