...
export const History = () => {
    ...
    const handleClearHistory = async () => {
        if (confirm(t('history.deleteConfirmAll'))) {
            try {
    ...
    return (
        <div className="max-w-5xl mx-auto mt-8 p-4 transition-colors duration-200">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-slate-800 dark:text-white">{t('history.title')}</h1>
                    <p className="text-slate-500 dark:text-slate-400 mt-2">{t('history.subtitle')}</p>
                </div>
                ...
                <div className="flex flex-wrap gap-3 w-full md:w-auto">
                    ...
                    <div className="relative flex items-center bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl px-3 transition-colors">
                        ...
                        >
                            <option value="date-desc">{t('history.sortRecent')}</option>
                            <option value="date-asc">{t('history.sortOldest')}</option>
                            <option value="az">{t('history.sortAZ')}</option>
                            <option value="za">{t('history.sortZA')}</option>
                        </select>
                    </div>
                    ...
                    {decisions.length > 0 && (
                        ...
                        >
                            <AlertOctagon className="w-4 h-4" /> {t('history.clearHistory')}
                        </button>
                    )}
                </div>
            </div>
            ...
            {loading ? (
                <div className="flex justify-center p-20 animate-pulse text-slate-400">{t('history.loading')}</div>
                            ...
                            >
                                <div className="flex items-center gap-5">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-3 mt-1 text-xs text-slate-400 dark:text-slate-500 font-medium">
                                            <span>{d.createdAt ? new Date(d.createdAt).toLocaleDateString() : t('history.noDate')}</span>
                                            <span className="w-1 h-1 bg-slate-200 dark:bg-slate-600 rounded-full"></span>
                                            <span>{d.criteria?.length || 0} {t('history.criteria')}</span>
                                            <span className="w-1 h-1 bg-slate-200 dark:bg-slate-600 rounded-full"></span>
                                            <span>{d.options?.length || 0} {t('history.optionsCount')}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-4">
                                    <span className={`text-[10px] uppercase tracking-widest font-bold px-3 py-1 rounded-full transition-colors ${isCompleted ? 'bg-emerald-100 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400' : 'bg-amber-100 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400'}`}>
                                        {isCompleted ? t('history.completed') : t('history.draft')}
                                    </span>
