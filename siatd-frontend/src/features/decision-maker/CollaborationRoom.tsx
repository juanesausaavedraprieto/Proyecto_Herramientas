...
export const CollaborationRoom = () => {
    ...
    useEffect(() => {
        if (!currentDecision && id) {
            api.get(`/decisions/${id}`)
                ...
                .catch(() => toast.error(t('decision.collab.loadError')));
        }
            ...
            client.subscribe(`/topic/decision/${id}`, (payload) => {
                ...
                if (message.type === 'JOIN') {
                    setParticipants(prev => {
                        ...
                        setMessages(m => [...m, { id: generateId(), sender: 'Sistema', content: `${message.userName} ${t('decision.collab.systemJoined')}`, type: 'SYSTEM', timestamp: new Date() }]);
                ...
                if (message.type === 'VOTE_SUBMITTED') {
                    ...
                    toast.success(`¡${message.userName} ${t('decision.collab.voteSubmitted')}`);
                }

                if (message.type === 'ROOM_CLOSED') {
                    toast.success(t('decision.collab.hostConsolidated'));
                ...
                if (message.type === 'CHAT_PRIVATE') {
                    if (message.targetUser === currentUserName && !myDndActive) {
                        ...
                        toast(t('decision.collab.privateMessage'), { description: `${t('decision.collab.privateFrom')} ${message.userName}` });
    ...
    const handleCopyLink = () => {
        ...
        toast.success(t('decision.collab.copyLink'));
    };
    ...
    const handleSendMessage = (e: React.FormEvent) => {
        ...
        if (chatTarget !== 'ALL') {
            ...
            if (targetPerson?.dndActive) {
                toast.error(`${chatTarget} ${t('decision.collab.dndError')}`);
                return;
            }
        }
    ...
    const toggleDnd = () => {
        ...
        toast.info(newState ? t('decision.collab.privacyOn') : t('decision.collab.privacyOff'));
    };
    ...
    const handleAiHelp = async () => {
        ...
        setIsAiLoading(true);
        toast.info(t('decision.collab.aiHelping'), { duration: 3000 });
        try {
            ...
            toast.success(t('decision.collab.aiFillSuccess'));
        } catch (e) {
            toast.error(t('decision.collab.aiFillError'));
        } finally {
    ...
    const handleProcessCollaborativeDecision = async () => {
        ...
        if (voters.length === 0) { toast.warning(t('decision.collab.noVotes')); return; }
        ...
        } catch (error) { toast.error(t('decision.collab.processingError')); } finally { setIsCalculating(false); }
    };
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 transition-colors">
                <div>
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-2xl font-black text-slate-800 dark:text-white">{t('decision.collab.title')}</h2>
                    </div>
                    <p className="text-slate-500 text-sm mt-1">{t('decision.collab.dilemma')} <span className="font-bold text-indigo-600">"{currentDecision.title}"</span></p>
                </div>
                <div className="flex gap-3">
                    <button onClick={toggleDnd} className={`flex items-center gap-2 font-bold text-sm py-2 px-4 rounded-xl transition-all border ${myDndActive ? 'bg-rose-50 text-rose-600 border-rose-200' : 'bg-slate-50 text-slate-600 border-slate-200'}`}>
                        <BellOff className="w-4 h-4" /> {myDndActive ? t('decision.collab.messagesBlocked') : t('decision.collab.receivingMessages')}
                    </button>
                    <button onClick={handleCopyLink} className="flex items-center gap-2 bg-indigo-50 text-indigo-600 font-bold text-sm py-2 px-4 rounded-xl">
                        <Share2 className="w-4 h-4" /> {t('decision.collab.invite')}
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm h-fit overflow-x-auto">
                        <div className="flex justify-between items-center mb-4 border-b border-slate-100 dark:border-slate-700 pb-3">
                            <h3 className="font-bold text-slate-800 dark:text-white flex items-center gap-2"><Calculator className="w-5 h-5 text-indigo-500" /> {t('decision.collab.yourEvaluation')}</h3>
                            <button onClick={handleAiHelp} disabled={isAiLoading || myVoteSubmitted} className="text-xs bg-indigo-100 text-indigo-600 font-bold px-3 py-1.5 rounded-lg flex items-center gap-1 disabled:opacity-50">
                                {isAiLoading ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />} {t('decision.collab.aiHelp')}
                            </button>
                        </div>
                        ...
                        <table className="w-full text-left text-sm">
                            <thead>
                                <tr>
                                    <th className="p-2 font-semibold text-slate-600 dark:text-slate-300">{t('decision.collab.optionCriterion')}</th>
                        ...
                        <button onClick={handleSubmitMyVotes} disabled={myVoteSubmitted || !isConnected} className="w-full mt-6 flex justify-center items-center gap-2 font-bold py-3 px-4 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white disabled:opacity-50">
                            {myVoteSubmitted ? <CheckCircle2 className="w-5 h-5" /> : t('decision.collab.submitVotes')}
                        </button>
                    </div>

                    {isHost && (
                        <div className="bg-emerald-50 dark:bg-emerald-900/20 p-6 rounded-3xl border border-emerald-100 dark:border-emerald-800/30">
                            <h3 className="font-bold text-emerald-800 dark:text-emerald-400 mb-2">{t('decision.collab.hostPanel')}</h3>
                ...
                <div className="lg:col-span-2 flex flex-col gap-6">
                    <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm">
                        <h3 className="font-bold text-slate-800 dark:text-white border-b border-slate-100 pb-3 mb-4 flex items-center gap-2"><Users className="w-5 h-5" /> {t('decision.collab.board')}</h3>
                        <ul className="space-y-2">
                            <li className="flex justify-between p-2 rounded-lg bg-slate-50 dark:bg-slate-900">
                                <span className="font-bold text-sm flex items-center gap-1">{currentUserName} {t('decision.collab.you')} {isHost && <Crown className="w-3 h-3 text-amber-500" />}</span>
                                <span className={`text-[10px] uppercase font-bold ${myVoteSubmitted ? 'text-emerald-600' : 'text-amber-500'}`}>{myVoteSubmitted ? t('decision.collab.submitted') : t('decision.collab.thinking')}</span>
                            </li>
                            {participants.filter(p => p.userName !== currentUserName).map((p, idx) => (
                                <li key={idx} className="flex justify-between p-2 rounded-lg border border-slate-100 dark:border-slate-700">
                                    ...
                                    <span className={`text-[10px] uppercase font-bold ${p.hasVoted ? 'text-emerald-600' : 'text-amber-500'}`}>{p.hasVoted ? t('decision.collab.submitted') : t('decision.collab.thinking')}</span>
                                </li>
                            ))}
                        </ul>
                    </div>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col flex-1 h-[400px]">
                        ...
                        <form onSubmit={handleSendMessage} className="p-3 border-t border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-b-3xl flex gap-2">
                            <select value={chatTarget} onChange={(e) => setChatTarget(e.target.value)} className="bg-slate-100 text-slate-700 text-xs font-bold rounded-lg px-2 outline-none w-24">
                                <option value="ALL">{t('decision.collab.chatPublic')}</option>
                            ...
                            <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} placeholder={t('decision.collab.chatPlaceholder')} className="flex-1 bg-slate-50 border border-slate-200 text-sm rounded-lg px-3 outline-none" />
